...
int my_colors [] = {kRed, kBlue, 8, kOrange+1, kViolet-3, 28, 15};
gErrorIgnoreLevel = kFatal;
gROOT->Macro("sub_macros/style.C");
void relative_outgassing(TString fname, TString option = "") {  // "comp" or "average"
   ...
   Double_t meanmass_err [nummeas], relative_throughput [nummeas], throughput_avg [nummeas], *yplot;
   ...
   for (int j=0;j<npeaks;j++) {
      ...
      for (int i=0;i<n_measurements;i++) {
         ...
         if (i==0) initial_throughput = conductance * fraction[i] * measurement_pressure[i];
         relative_throughput[i] = conductance * fraction[i] * measurement_pressure[i] / initial_throughput;
         err_sys[i] = stat_factor * relative_throughput[i];
         if (option == "average") {
      ...
      else yplot = relative_throughput;
      ...
      mg->Add(garray[j]);
      leg->AddEntry(garray[j], graphname);
...
//    c1->SetLogy();
   ...
   mg->GetYaxis()->SetTitle("Fraction of Initial Outgassing");
   if (print==1 && area == 1) c1->Print(file_header + "initial_findings/" + img_name + "_rel_int.png");

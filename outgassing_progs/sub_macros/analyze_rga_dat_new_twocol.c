     ...
     for (int i=0; i<(maxbins-1); i++) {
       while (getline (infile1,params)) {
         if (ii==0) {
            ...
            sscanf(params.c_str(),"%le %le",&xax,&fy);
         }
         num[i] = fnum;
         y[i] = fy;
         x[i] = xax;
...
rga_scan analyze_rga_dat_new_twocol(TString fname, Double_t input_mass = 0, TString getpeaks = "") //gets composition analysis from rga
{
//    Double_t scan_time;
